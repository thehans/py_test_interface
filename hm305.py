...
def digit_str(value, max_decimals, disp_digits=4):
    ...
    int_digits = 1 if value < 10 else (2 if value < 100 else 3) # digits before decimal point
    decimal_avail = min(disp_digits - int_digits, max_decimals)
    return "%*.*f"  % (disp_digits+1, decimal_avail, value) # +1 total width for decimal point
...
class Register(Enum):
    ...
    SetTimeSpan  = 0x0032 # Time Span setting pulled from last selected M preset. List mode only.
...
if __name__ == "__main__":
    ...
    while True:
        ...
        v,i,p = digit_str(hm.v, hm.v_decimals), digit_str(hm.i, hm.i_decimals), digit_str(hm.p, hm.p_decimals)
